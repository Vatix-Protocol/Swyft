...
export function RemoveLiquidityPanel({
  ...
  useEffect(() => {
    ...
    estimateRemoveAmountsAsync({
      ...
      .then((r) => {
        if (!cancelled) setEstimates(r);
      })
      .catch(() => {
        if (!cancelled) setEstimates({ amount0: '0', amount1: '0' });
      })
      .finally(() => {
          ...
          <div className="flex flex-col gap-1 text-xs text-indigo-600 dark:text-indigo-300">
            <div className="flex justify-between">
              ...
              <span className="font-medium tabular-nums">
                {parseFloat(position.uncollectedFeesToken0).toFixed(7)}
              </span>
            </div>
            <div className="flex justify-between">
              <span>{token1Symbol}</span>
              <span className="font-medium tabular-nums">
                {parseFloat(position.uncollectedFeesToken1).toFixed(7)}
              </span>
            </div>
          </div>
        </div>
